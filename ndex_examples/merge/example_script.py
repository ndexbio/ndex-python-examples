...
local = nc.Ndex("http://dev.ndexbio.org", "bindingdb", "bindingdb")
local.set_debug_mode(True)
...
ns = local.save_new_network(merged_network)
merged_network_id = ns.get('externalId')

bindingdb_provenance = local.get_provenance(bindingdb_id)
my_provenance = local.get_provenance(my_network_id)
new_network_provenance = local.get_provenance(merged_network_id)
merged_provenance = merger.merge_provenance(my_provenance, bindingdb_provenance, new_network_provenance)
local.set_provenance(merged_network_id, merged_provenance)
